...
create policy "read_all_auth" on public.ai_actions
  for select to authenticated using (true);

create policy "insert_auth" on public.ai_actions
  for insert to authenticated with check (true);

create policy "update_auth" on public.ai_actions
  for update to authenticated using (true) with check (true);

create policy "delete_auth" on public.ai_actions
  for delete to authenticated using (true);

create index if not exists ai_actions_lead_created_idx
  on public.ai_actions (lead_id, created_at desc);

create index if not exists ai_actions_conversation_created_idx
